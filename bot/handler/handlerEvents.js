...
async function trackCommandSpam(threadID, threadName, globalData, message) {
        ...
        if (result.shouldBan) {
                ...
                message.reply(`⛔ | Ce groupe a été temporairement banni pendant ${hours} heures en raison d'un spam de commandes.\n\nVeuillez patienter ou contacter un administrateur pour lever le bannissement.`);
                ...
                const parameters = {
                        ...
                        removeCommandNameFromBody: function removeCommandNameFromBody(body_, prefix_, commandName_) {
                                if ([body_, prefix_, commandName_].every(x => nullAndUndefined.includes(x)))
                                        throw new Error("Veuillez fournir 'body', 'prefix' et 'commandName' pour utiliser cette fonction. L'appel sans paramètres n'est pris en charge que pour 'onStart'.");
                                for (let i = 0; i < arguments.length; i++)
                                        if (typeof arguments[i] != "string")
                                                throw new Error(`Le paramètre "${i + 1}" doit être une chaîne, mais a reçu "${getType(arguments[i])}"`);
                ...
                async function onStart() {
                        ...
                        if (isGroup) {
                                ...
                                if (isSpamBanned) {
                                        if (!hideNotiMessage.threadBanned)
                                                message.reply("Cette conversation est temporairement restreinte en raison d'un envoi massif de commandes. Veuillez patienter un moment et réessayer plus tard.");
                                        return;
                                }
                        }
                        ...
                        if (!command) {
                                ...
                                if (!hideNotiMessage.commandNotFound) {
                                        if (!commandName) {
                                                return await message.reply(`Ce n'est que le préfixe. Tapez ${prefix}help pour voir les commandes.😽🍓`);
                                        }
                                        ...
                                        if (bestMatch) {
                                                suggestionMsg += ` 😽🍓Essayez : ${prefix}${bestMatch}`;
                                        }
                        ...
                        if (requiredMoney && requiredMoney > 0) {
                                ...
                                if (!hasEnoughMoney) {
                                        ...
                                        return await message.reply(
                                                `Vous devez avoir au moins $${requiredMoney} pour utiliser cette commande.\n` +
                                                `Votre solde : $${userMoney}\n` +
                                                `Manquant : $${requiredMoney - userMoney}`
                                        );
                                }
                        }
                        ...
                        if (!canUseCommand(role, needRole)) {
                                if (!hideNotiMessage.needRoleToUseCmd) {
                                        if (needRole == 1)
                                                ...
                                                return await message.reply("Cette commande nécessite un accès premium. Veuillez passer à la version premium pour continuer😽🍓.");
                                        else if (needRole == 4)
                                                return await message.reply("Cette fonctionnalité est réservée à mon bebe 😽🍓.");
                                        else
                                                return await message.reply("Vous n'avez pas la permission d'utiliser cette commande😾.");
                                }
                                else {
                                        return true;
                                }
                        ...
                        if (!canUseCommand(role, needRole)) {
                                if (!hideNotiMessage.needRoleToUseCmdOnReply) {
                                        if (needRole == 1)
                                                ...
                                                return await message.reply("Cette commande est disponible uniquement pour les membres premium. Veuillez mettre à niveau votre accès pour continuer😽🍓.");
                                        else if (needRole == 4)
                                                return await message.reply("Cette fonctionnalité est réservée à mon mamou S64 ASH 😽🍓.");
                                }
                                else {
                                        return true;
                                }
                        ...
                        if (!canUseCommand(role, needRole)) {
                                if (!hideNotiMessage.needRoleToUseCmdOnReaction) {
                                        if (needRole == 1)
                                                ...
                                                return await message.reply("Laisse moi tranquille connard😒.");
                                }
                                else {
                                        return true;
                                }
